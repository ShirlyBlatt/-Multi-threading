...
public abstract class MicroService implements Runnable {
    ...
    private MessageBusImpl msgBusInstance = MessageBusImpl.getInstance();
    private boolean terminated = false;
    private final String name;
    private HashMap<Class<? extends Message>, Callback<?>> messageToCallback;
    ...
    public MicroService(String name) {
        this.name = name;
        this.messageToCallback = new HashMap<Class<? extends Message>, Callback<?>>();
    }
    ...
    protected final <T, E extends Event<T>> void subscribeEvent(Class<E> type, Callback<E> callback) {
        msgBusInstance.subscribeEvent(type, this);
    ...
    protected final <B extends Broadcast> void subscribeBroadcast(Class<B> type, Callback<B> callback) {
        msgBusInstance.subscribeBroadcast(type, this);
        messageToCallback.put(type, callback);
    }
    ...
    protected final <T> Future<T> sendEvent(Event<T> e) {
        return msgBusInstance.sendEvent(e);
    }
    ...
    protected final void sendBroadcast(Broadcast b) {
        msgBusInstance.sendBroadcast(b);
    }
    ...
    protected final <T> void complete(Event<T> e, T result) {
        msgBusInstance.complete(e, result);
    }
    ...
    public final void run() {
        msgBusInstance.register(this);
        initialize();
        Message message = null;
        while (!terminated) {
            try {
                message = msgBusInstance.awaitMessage(this);
//                if (message.getClass() == TickBroadcast.class && message.getData().equals(-1)){
//                    System.out.println(this.name + " terminated");
//                }
                // TODO: should we wait here or in the await message
            } catch (InterruptedException e) {
                e.printStackTrace();
            }
            if (message != null){
                Callback callback = this.messageToCallback.get(message.getClass());
                callback.call(message);
            }

        }
        msgBusInstance.unregister(this);
    }
